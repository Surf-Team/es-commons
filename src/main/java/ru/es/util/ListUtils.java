...
import ru.es.annotation.Description;
...
public class ListUtils
{
    ...
    private static Object createInstance(String v, Class neadedClass)
    {
        ...
        else if (neadedClass == Float.class)
    ...
    public static boolean containsEqualIgnoreCase(Iterable<String> list, String text)
    {
        for (var str : list)
        {
    ...
    // создаёт список из списка, в котором есть другой список
    public static <Outer, Inner> List<Inner> flatList(List<Outer> list, Function<Outer, List<Inner>> function)
    {
        List<Inner> ret = new ArrayList<>();
        for (Outer o : list)
        {
            ret.addAll(function.apply(o));
        }
        return ret;
    }
}
