...
public class AnnotatedMap
{
	...
	// можно использовать аннотацию Key чтобы сформировать мапу из списка
	public static <K, T> Map<K, T> createKMap(List<T> items)
	{
	...
	public static <T> Map<String, Map<?, T>> createMaps(List<T> items)
	{
		Map<String, Map<?, T>> maps = new HashMap<>();
		if (items.size() > 0)
		{
			Class c = items.get(0).getClass();
			for (Field f : c.getFields())
			{
				for (Annotation a : f.getAnnotations())
				{
					if (a instanceof UniqueKey)
					{
						Map<?, T> map = AnnotatedMap.createMap(items, f);
